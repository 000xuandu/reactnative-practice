...
function TodoScreen() {
  const { todoIdList, renderItem, keyExtractor, addTodoItem } = useTodoScreenLogic();

  return (
    <SafeAreaView style={styles.container}>
      <FlatList renderItem={renderItem} keyExtractor={keyExtractor} data={todoIdList} />
...
const useTodoScreenLogic = () => {
  ...
  const todoIdList = useAppSelector(
    (rootState: RootState) => rootState.countReducer.list.map((item: Todo) => item.id)
    // shallowEqual,
  );

  const keyExtractor = (item: any) => item.toString();

  const renderItem = ({ item }: { item: number }) => <TodoItemMemo key={item} id={item} />;
